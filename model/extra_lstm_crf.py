...
class Extra_LSTM_CRF(torch.nn.Module):
    def __init__(self, mixsize, extra_dim, n_word, word_dim,
                 n_layers, word_hidden, n_target, drop=0.5):
        super(Extra_LSTM_CRF, self).__init__()
        ...
        self.scalarmix = ScalarMix(extra_dim, mixsize, False)
    
        if n_layers > 1:
            self.lstm_layer = torch.nn.LSTM(
                input_size=self.embedding_dim + extra_dim,
    ...
    def forward(self, word_idxs, extras):
        # mask = torch.arange(x.size()[1]) < lens.unsqueeze(-1)
        mask = word_idxs.gt(0)
        sen_lens = mask.sum(1)
        
        extra_feature = torch.split(extras, 1, dim=2)
        extra_feature = self.scalarmix(extra_feature)
        
        word_vec = self.embedding(word_idxs)
        feature = self.drop1(torch.cat((word_vec, extra_feature), -1))
    ...
    def forward_batch(self, batch):
        word_idxs, extras, label_idxs = batch
        mask = word_idxs.gt(0)
        out = self.forward(word_idxs, extras)
